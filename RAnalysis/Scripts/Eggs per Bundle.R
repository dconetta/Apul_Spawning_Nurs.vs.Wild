...
avg.eggs <- aggregate(Num.Eggs ~ Coral.ID*Condition, data = eggs.per.bundle, FUN = mean)
boxplot(Num.Eggs ~ Condition, data = avg.eggs, xlab = 'Coral Colony Origin', ylab = 'Eggs per Bundle', col = c("red", "blue"))

dev.off()

#stats
t.test(Num.Eggs~Condition, data = avg.eggs) #Statistically significant if p-value <0.05
